...
def modify_database_view(request, site_id):
    ...
    return render(
        request,
        "sites/edit_database.html",
        {"site": site, "sql_version": get_sql_version(site)},
    )
...
def sql_database_view(request, site_id):
    ...
    if site.database.category == "mysql":
        if not settings.MYSQL_DB_HOST:
            return HttpResponse(
                "Director has been improperly configured! (Missing MYSQL_DB_HOST)",
                content_type="text/plain",
            )
        ...
        ret, out, err = run_as_site(
            ...
            [
                "mysql",
                "--user={}".format(site.database.username),
                "--host={}".format(settings.MYSQL_DB_HOST),
                site.database.db_name,
                "-e",
                sql,
            ],
            env={"MYSQL_PWD": site.database.password},
        )
    else:
        if not settings.POSTGRES_DB_HOST:
            return HttpResponse(
                "Director has been improperly configured! (Missing POSTGRES_DB_HOST)",
                content_type="text/plain",
            )

        ret, out, err = run_as_site(
            site,
            ["psql", str(site.database), "-c", sql],
    ...
    if site.database.category == "postgresql":
        ...
        proc = Popen(
            [
                ...
                "--password={}".format(site.database.password),
                "-h",
...
def delete_database_view(request, site_id):
    ...
    if request.method == "POST":
        ...
        return render(
            request,
            "sites/delete_database.html",
            {"site": site, "tables": list_tables(site.database)},
        )
