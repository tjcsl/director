...
class SiteForm(forms.ModelForm):
    ...
    custom_nginx = forms.BooleanField(
        required=False,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance and instance.pk:
            ...
            if instance.purpose == "legacy" or (
                hasattr(self, "_user") and self._user.is_superuser
            ):
    ...
    def clean_domain(self):
        ...
        for domain in data:
            if domain.endswith("tjhsst.edu"):
                ...
                if (
                    not domain[: -len(settings.PROJECT_DOMAIN)].rstrip(".")
                    == self.cleaned_data["name"]
                ):
                    if not self._user.is_superuser:
                        raise forms.ValidationError(
                            "Your subdomain for {} must match your site name!".format(
                                settings.PROJECT_DOMAIN
                            )
                        )
            else:
                if (
                    Domain.objects.filter(domain=domain)
                    .exclude(site__name=self.cleaned_data["name"])
                    .exists()
                ):
                    raise forms.ValidationError(
                        "The domain {} is already taken by another site! If you believe this is an error, please send an email to {}.".format(
        ...
        if "domain" in cleaned_data:
            if cleaned_data["purpose"] in ["user", "activity", "legacy"]:
                if default_domain not in cleaned_data["domain"]:
                    raise forms.ValidationError(
                        "Sites of type '{}' must keep the default '{}' domain!".format(
...
class ProcessForm(forms.ModelForm):
    ...
    def clean_path(self):
        ...
        if not value.startswith(root_path):
            raise forms.ValidationError(
                "The script you are trying to reference must be in the {} folder!".format(
                    root_path
                )
            )
...
class DatabaseForm(forms.ModelForm):
    ...
    host = forms.ModelChoiceField(
        queryset=DatabaseHost.objects.all(),
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if not user.is_superuser:
            self.fields["site"].queryset = Site.objects.exclude(
                category="static"
            ).filter(group__users__id=user.id)
