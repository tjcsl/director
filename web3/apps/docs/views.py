...
def read_article_view(request, article_slug, revision_id=None):
    ...
    if revision_id is not None:
            ...
            return JsonResponse(
                {
                    ...
                    "tags": list(
                        public_article.tags.values_list("name", flat=True).order_by(
                            "name"
                        )
                    ),
                }
        ...
        article = get_object_or_404(
            Article, slug=article_slug, publish_id__isnull=False
        )
        public_article = article.published_article
    return render(
        request, "docs/read.html", {"article": public_article, "revision": revision_id}
    )
...
def list_articles_view(request):
    ...
    if "all" in request.GET and request.user.is_staff:
        ...
        available_articles = Article.objects.filter(
            Q(publish_id__isnull=False) | Q(author=request.user)
        )
    else:
    ...
    return render(request, "docs/list.html", {"articles": public_articles})
...
def index_view(request):
    ...
    tags = (
        Tag.objects.filter(article__publish_id__isnull=False)
        .order_by("name")
        .distinct()
    )
...
def article_history_view(request, article_slug):
    ...
    return render(
        request,
        "docs/history.html",
        {"article": article, "revisions": revisions, "publish_id": article.publish_id},
    )
...
def save_history_view(request, article_slug):
    ...
    if form.is_valid():
        ...
        return JsonResponse(
            {
                "success": "Successfully created revision with ID #{}".format(
                    article.history.first().history_id
                ),
